...
alg = args.Alg
log_dir_name = 'logs/' + time_str + '_' + 'old' + '_' + alg + args.Info + '_n' + \
...
class DPDS:
    ...
    def cost(self, batch_state, batch_action):
        ...
        if args.Alg == 'dpl':
            ...
            cost = tf.reduce_sum(
                batch_state[:, :, 2] - d / ((param.d_lb + param.d_ub) / 2) + self.lam * tf.math.maximum(0.0,
                                                                                                        E - self.E_max),
                axis=1)
        else:
            cost = tf.reduce_sum(
                batch_state[:, :, 1] - d * aoi_per_bit + self.lam * tf.math.maximum(0.0, E - self.E_max), axis=1)
